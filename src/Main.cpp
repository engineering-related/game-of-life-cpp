...
layout(rgba32f, binding = 1) uniform image2D screen_old;
...
int main()
{
	...
	GLuint screenTexOld;
	glCreateTextures(GL_TEXTURE_2D, 1, &screenTexOld);
	glTextureParameteri(screenTexOld, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTextureParameteri(screenTexOld, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTextureParameteri(screenTexOld, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTextureParameteri(screenTexOld, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTextureStorage2D(screenTexOld, 1, GL_RGBA32F, SCREEN_WIDTH, SCREEN_HEIGHT);
	glBindImageTexture(1, screenTexOld, 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
